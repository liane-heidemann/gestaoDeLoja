...
function printArray(itensDoEstoque){ 
  for(var i=0; i<itensDoEstoque.length; i++) {
    console.log((i+1)+") - Produto: "+itensDoEstoque[i].produto
    +", Preço: "+itensDoEstoque[i].valor+", Quantidade: "
    +itensDoEstoque[i].quantidade);
  }
}
...
function removerItemNoEstoque() { 
  ...
  const idxObj = itensDoEstoque.findIndex(object => {return object.produto === removerProduto;});
